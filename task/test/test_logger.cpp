...
int main()
{
	...
	if(!tools::load_logger_config(PROJECT_SOURCE_DIR "/config/tools/logger_config.toml", config,
	                              &config_error))
	{
		std::cerr << "logger config load failed: " << config_error << '\n';
		return 1;
	}

	if(config.file_path != "data/logs/robocore.log")
	{
		std::cerr << "unexpected logger file path: " << config.file_path << '\n';
		return 2;
	}
	...
	if(!file_contains(log_path, "[ERROR] - /dev/ttyACM0 open failed: permission denied"))
	{
