/**
 * @file test_tracker_visual.cpp
 * @brief 在 demo.avi 上可视化 Tracker 车辆级跟踪结果（人工回放，不注册 CTest）。
 *
 * 数据流：
 ...
 * - 车辆速度向量（洋红箭头）
 * - 各预测 armor hypothesis（品红圆点 + armor_id；matched armor 高亮为红色粗圈）
 * - Tracker 状态 / has_measurement / yaw / yaw_rate / radius / innovation / NIS 文本
 *
...
namespace
{
	...
	constexpr const char* kWindowName = "tracker_visual";
	...
	const cv::Scalar kMatchedColor{0, 0, 255};           // 红
	...
	void draw_cross(cv::Mat& image, const cv::Point& center, const cv::Scalar& color, int radius)
	{
		cv::line(image, cv::Point(center.x - radius, center.y),
		         cv::Point(center.x + radius, center.y), color, 2);
		cv::line(image, cv::Point(center.x, center.y - radius),
		         cv::Point(center.x, center.y + radius), color, 2);
	}

	std::vector<std::string> normalize_args(int argc, char** argv)
	{
		const std::vector<std::string> value_options = {"--video", "--quaternion"};
...
int main(int argc, char** argv)
{
	...
	if(!video.isOpened())
	{
		std::printf("[TRACKER_VISUAL] [ERROR] failed to open video: %s\n", video_path.c_str());
		return 2;
	}
	...
	{
		std::printf("[TRACKER_VISUAL] [WARN] no synchronized quaternion; world projection uses identity\n");
	}
	...
	{
		std::printf("[TRACKER_VISUAL] [ERROR] failed to load detector config\n");
		return 3;
	}
	...
	{
		std::printf("[TRACKER_VISUAL] [ERROR] failed to load solver config\n");
		return 4;
	}
	...
	{
		std::printf("[TRACKER_VISUAL] [ERROR] OpenVINO inference is not ready\n");
		return 5;
	}
	...
	{
		std::printf("[TRACKER_VISUAL] [ERROR] AutoAim is not ready\n");
		return 6;
	}
	...
	auto run_one_frame = [&]() -> bool {
		...
		if(result.tracked_target.has_value())
		{
			...
			for(const auto& hypothesis: tracked.predicted_armors)
			{
				...
				const bool is_matched = tracked.matched_armor_id.has_value()
				    && *tracked.matched_armor_id == hypothesis.armor_id;

				const cv::Point center(static_cast<int>(pixel->x), static_cast<int>(pixel->y));
				const cv::Scalar color = is_matched ? kMatchedColor : kHypothesisColor;
				const int radius = is_matched ? 7 : 4;

				cv::circle(canvas, center, radius, color, is_matched ? 3 : 1);

				char label[16];
				std::snprintf(label, sizeof(label), "%d", hypothesis.armor_id);
				cv::putText(canvas, label, cv::Point(center.x + radius + 2, center.y),
			...
			char state_line[128];
			std::snprintf(state_line, sizeof(state_line), "tracker: %s  has_meas=%d",
			              tracker_state_name(tracked.state), tracked.has_measurement ? 1 : 0);
			cv::putText(canvas, state_line, origin, cv::FONT_HERSHEY_SIMPLEX, 0.6, kTextColor, 1);

			char pose_line[256];
			std::snprintf(pose_line, sizeof(pose_line),
			...
			cv::putText(canvas, pose_line, cv::Point(origin.x, origin.y + 22),
			            cv::FONT_HERSHEY_SIMPLEX, 0.6, kTextColor, 1);

			char geom_line[256];
			std::snprintf(geom_line, sizeof(geom_line),
			              "yaw=%.3f  yaw_rate=%.3f  radius=%.3f  dR=%.3f  dZ=%.3f",
			              tracked.yaw, tracked.yaw_rate, tracked.radius, tracked.delta_radius,
			              tracked.delta_z);
			cv::putText(canvas, geom_line, cv::Point(origin.x, origin.y + 44),
			            cv::FONT_HERSHEY_SIMPLEX, 0.6, kTextColor, 1);

			if(tracked.matched_armor_id.has_value())
			{
				char match_line[128];
				std::snprintf(match_line, sizeof(match_line), "matched_armor=%d",
				              *tracked.matched_armor_id);
				cv::putText(canvas, match_line, cv::Point(origin.x, origin.y + 66),
				            cv::FONT_HERSHEY_SIMPLEX, 0.6, kMatchedColor, 1);
			}

			if(tracked.nis.has_value())
			{
				char nis_line[128];
				std::snprintf(nis_line, sizeof(nis_line), "NIS=%.3f", *tracked.nis);
				cv::putText(canvas, nis_line, cv::Point(origin.x, origin.y + 88),
				            cv::FONT_HERSHEY_SIMPLEX, 0.6, kTextColor, 1);
			}
		}
		else
		{
			cv::putText(canvas, "tracker: Lost", cv::Point(10, 30),
	...
	std::printf("[TRACKER_VISUAL] processed %zu frames\n", frame_index);
	return 0;
}