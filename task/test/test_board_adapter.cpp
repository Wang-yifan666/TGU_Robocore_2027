...
namespace
{
    ...
    void test_bullet_speed_mapping(TestRunner& runner)
    {
        ...
        const Eigen::Matrix3d r_gi = Eigen::Matrix3d::Identity();

        {
            task::BoardFeedback board;
            board.has_bullet_speed = true;
            board.bullet_speed_mps = 23.0;
    ...
    void test_gimbal_yaw_priority_and_missing(TestRunner& runner)
    {
        runner.begin("gimbal yaw priority and missing");
...

        {
            task::BoardFeedback board;
            board.has_quaternion = true;
            board.q_imu_body_to_world = Eigen::AngleAxisd(0.3, Eigen::Vector3d::UnitZ());
            board.has_gimbal_yaw = true;
            board.gimbal_yaw_rad = 0.7;
            const auto frame = task::make_frame_context(camera, board, identity);
            runner.expect(near(frame.gimbal_yaw_rad, 0.7), "has_gimbal_yaw wins");
        }
        {
            task::BoardFeedback board;
            const auto frame = task::make_frame_context(camera, board, identity);
            runner.expect(std::isnan(frame.gimbal_yaw_rad), "missing both -> NaN");
        }
        {
            task::BoardFeedback board;
            board.has_gimbal_yaw = true;
            board.gimbal_yaw_rad = kPi + 0.1;
            const auto frame = task::make_frame_context(camera, board, identity);
            runner.expect(near(frame.gimbal_yaw_rad, -kPi + 0.1), "wrapped to [-pi, pi)");
        }
...
int main()
{
    ...
    test_gimbal_yaw_priority_and_missing(runner);
