...
bool load_logger_config(const std::filesystem::path& config_path,
                        LoggerConfig& config,
    ...
    try {
        ...
        if (const auto level_text =
                logger_table["level"].value<std::string>()) {
            const auto parsed_level = parse_log_level(*level_text);

            if (!parsed_level.has_value()) {
                set_error_message(
                    error_message,
                    "invalid logger.level: " + *level_text);
                return false;
            }
        ...
        candidate.enable_file =
            logger_table["enable_file"].value_or(candidate.enable_file);

        candidate.flush_on_error =
            logger_table["flush_on_error"].value_or(candidate.flush_on_error);

        if (const auto file_path =
                logger_table["file_path"].value<std::string>()) {
        ...
        stream << "failed to parse " << config_path.string()
               << ": " << error.description();
