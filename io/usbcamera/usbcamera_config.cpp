...
namespace io
{
	...
	bool load_usb_camera_config(const std::filesystem::path& config_path, UsbCameraConfig& config,
		...
		try
		{
			...
			// ---- device (string, must not be empty) ----
			{
				...
				if(node)
				{
					...
					if(!val)
					{
						set_error_message(error_message, "usb_camera.device must be a string");
						return false;
					}
					if(val->empty())
					{
						set_error_message(error_message, "usb_camera.device cannot be empty");
						return false;
					}
			...
			// ---- width (int, > 0) ----
			{
				...
				if(node)
				{
					...
					if(!val)
					{
						set_error_message(error_message, "usb_camera.width must be an integer");
						return false;
					}
					if(*val <= 0)
					{
			...
			// ---- height (int, > 0) ----
			{
				...
				if(node)
				{
					...
					if(!val)
					{
						set_error_message(error_message, "usb_camera.height must be an integer");
						return false;
					}
					if(*val <= 0)
					{
			...
			// ---- fps (int, > 0) ----
			{
				...
				if(node)
				{
					...
					if(!val)
					{
						set_error_message(error_message, "usb_camera.fps must be an integer");
						return false;
					}
					if(*val <= 0)
					{
			...
			// ---- pixel_format (string, "MJPG" or "YUYV") ----
			{
				...
				if(node)
				{
					...
					if(!val)
					{
						set_error_message(error_message, "usb_camera.pixel_format must be a string");
						return false;
					}
					if(*val != "MJPG" && *val != "YUYV")
					{
						set_error_message(
						    error_message,
						    "usb_camera.pixel_format must be \"MJPG\" or \"YUYV\", got \"" + *val
						        + "\"");
						return false;
					}
			...
			// ---- buffer_size (int, > 0) ----
			{
				...
				if(node)
				{
					...
					if(*val <= 0)
					{
						set_error_message(
						    error_message,
						    "usb_camera.buffer_size must be > 0, got " + std::to_string(*val));
						return false;
					}
